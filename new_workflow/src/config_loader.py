...
def load_config(config_path: str = None, force_reload: bool = False) -> Dict[str, Any]:
    ...
    if _config_cache is not None and not force_reload:
        # 如果指定了路径，检查是否与缓存路径一致
        if config_path is None or config_path == _config_path_cache:
...
def get_config(key: str, default: Any = None) -> Any:
    """获取配置项的值，支持嵌套键（使用点号分隔），使用缓存提高性能"""
    try:
        config = load_config()  # 会自动使用缓存
        keys = key.split('.')
        value = config
        for k in keys:
            value = value[k]
